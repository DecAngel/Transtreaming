...
class BasicBlock_3x3_Reverse_DCN(nn.Module):
    def __init__(self,
                 ch_in,
                 ch_hidden_ratio,
                 ch_out,
                 act='relu',
                 shortcut=True):
        super(BasicBlock_3x3_Reverse_DCN, self).__init__()
        assert ch_in == ch_out
        ch_hidden = int(ch_in * ch_hidden_ratio)
        # self.conv1 = ConvBNAct(ch_hidden, ch_out, 3, stride=1, act=act)
            ...
            if block_fn == 'BasicBlock_3x3_Reverse':
                self.convs.add_module(
                    ...
                    (BasicBlock_3x3_Reverse(next_ch_in,
                                           ...
                                           shortcut=True)
                    if not use_dcn else
                    BasicBlock_3x3_Reverse_DCN(next_ch_in,
                                               ch_hidden_ratio,
                                               ch_mid,
                                               act=act,
                                               shortcut=True)
...
class GiraffeNeckV2(nn.Module):
    ...
    def forward(self, out_features):
        ...
        x34 = F.interpolate(x3, size=x1.shape[2:4], mode='nearest')
        ...
        x45 = F.interpolate(x4, size=x2.shape[2:4], mode='nearest')
