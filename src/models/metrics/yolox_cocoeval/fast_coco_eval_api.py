...
class COCOeval_opt(COCOeval):
    ...
    def evaluate(self):
        ...
        log.info("Running per image evaluation...")
        p = self.params
        ...
        log.info("Evaluate annotation type *{}*".format(p.iouType))
        ...
        toc = time.time()
        log.info("COCOeval_opt.evaluate() finished in {:0.2f} seconds.".format(toc - tic))
    ...
    def accumulate(self):
        ...
        """
        log.info("Accumulating evaluation results...")
        tic = time.time()
        if not hasattr(self, "_evalImgs_cpp"):
            log.info("Please run evaluate() first")
