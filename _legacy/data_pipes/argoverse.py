...
from typing import Union, Tuple, Literal, List, Sequence, Dict, Any
...
from .base import DataPipe, BatchDict, MetaDict, ImageDict, BBoxDict

logger = RankedLogger(__name__, rank_zero_only=False)


class ArgoverseDataPipe(DataPipe):
    def __init__(
            self,
            img_dir: str,
            ann_file: str,
    ...
    def _get_meta(self, seq_id: int, frame_id: int) -> MetaDict:
        return MetaDict(
    ...
    def _get_image(self, seq_id: int, frame_id: int) -> ImageDict:
    ...
    def _get_bbox(self, seq_id: int, frame_id: int) -> BBoxDict:
        image_id_0 = self._get_image_id_0(seq_id, frame_id)
        return BBoxDict(
    ...
    def __getitem__(self, item: Tuple[int, ...]) -> BatchDict:
        seq_id, frame_id, component_id = item
        if component_id == 0:
            return {'meta': self._get_meta(seq_id, frame_id)}
        elif component_id == 1:
            return {'image': self._get_image(seq_id, frame_id)}
        elif component_id == 2:
            return {'bbox': self._get_bbox(seq_id, frame_id)}
        else:
            raise ValueError(f'')

    def __len__(self) -> Tuple[Union[int, List[int]], ...]:
        return [len(self.seq_lens)], self.seq_lens, 3
